...
interface AdminUser { id: string; email: string | null; username: string | null; role: 'user' | 'admin'; created_at: string; }
...
export function AdminPanel({ onClose, onChanged, onLocate }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      let request = client.from('profiles').select('id,email,username,role,created_at').order('created_at', { ascending: false }).limit(100);
